      ...
      if (msg.event_type === "best_bid_ask" || msg.type === "best_bid_ask") {
        ...
        if (changed) {
          ...
          const toStopLoss = state.trades.filter(t => {
            ...
            if (Date.now() - t.entryTime < stopGraceMs) {
          ...
          const toTakeProfit = state.trades.filter(t => {
            if (t.tokenId !== tokenId) return false;
            // Near-certainty exit: at 97¢ the market has essentially resolved.
...
async function _runCryptoCycleInner(asset, { fastOnly = false } = {}) {
  ...
  for (const market of fresh) {
    ...
    // Respects: stress hold, direction veto, position limit, budget.
    {
      ...
      if (!fSnap?.flashEntered && windowAge < 45_000 && timeRemaining > 150) {
        ...
        if (
          ...
          // Always fall through to AI — log the flash signal so the AI has context.
          if (!fSnap.flashBlockLogged) {
            fSnap.flashBlockLogged = true;
            logEntry("dim",
              `  ↳ <span class="amber">flash signal</span> — gap ${gap >= 0 ? "+" : ""}$${gap.toFixed(pd)} (${(flashGapPct*100).toFixed(2)}%), ` +
              `${flashConfirmCnt}/3 signals (mom${flashMomOk ? "✓" : "✗"} candles ${flashAlignedCnt}/5 book${flashStrongBook ? "✓" : "✗"}) — routing to AI`
            );
          }
          // fall through to oracle gate / AI analysis
        }
      }
    }

    // Fast discovery cycle ends here — flash signal logging has run. Skip oracle gate and AI
    // analysis (handled by the 15s full cycle where AI makes the final entry decision).
    ...
    if (timeRemaining < 90) {
      ...
      // Log once per market — the tooCloseLogged flag prevents spam on re-queued FOK markets.
      if (!snap?.tooCloseLogged) {
        if (snap) snap.tooCloseLogged = true;
    ...
    if (qualifies) {
      ...
      if (windowAge < earlyGate) {
        ...
        const maxBetCfg    = c[`${asset}MaxBet`] ?? c.btcMaxBet ?? 5;
        ...
        if (probeTokenId) {
          try {
            ...
            if (!r.ok) {
              bookProbeReason = `price ${r.status}`;
            } else {
  ...
  if (c.dryRun) {
    ...
    const handleBuyResult = (result, isRetry) => {
      ...
      if (result.error) {
        ...
        if ((analysis.timeRemaining ?? 0) > 120) state[asset].gapWatch.set(market.conditionId, true);
